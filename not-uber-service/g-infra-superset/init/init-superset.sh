...
# The datasets, charts and dashboard live in assets/ as plain YAML, written
# by render_assets.py the same way the Grafana dashboards are. --overwrite
...
# to, and its uri has no password in it - a password in a committed file is
# not an option. --overwrite applies that passwordless uri to the
# connection, so whatever ran before it loses its password. Registering
# afterwards is what makes the credential survive; reversing these two made
# every chart fail with ClickHouse code 516 once already.
